...
console.log(
  chalk.bgYellowBright.bold.cyanBright(
    `\t><><><><><><><><><><  Wellcome to NBP ATM   ><><><><><><><><><`
  )
);
...
while (condition) {
  ...
  if ((pinAnswer.pin = myPinCode)) {
    console.log(
      chalk.bold.bgGreenBright("\t\tWellcome! you are sign in to your ATM.")
    );
    ...
    let operationAnswer = await inquirer.prompt([
      {
        ...
        message: chalk.blueBright(
          "Please select an option below to perform operations."
        ),
        type: "list",
        choices: [
          "Withdraw",
    ...
    if (operationAnswer.operation === "Withdraw") {
      ...
      if (amountAnswers.amount <= myBalance && amountAnswers.amount > 0) {
        ...
        console.log(
          chalk.greenBright(
            `You have succefully withdraw an amount of: Rs.${amountAnswers.amount} Your new balance is: Rs.${myBalance}`
          )
        );
      } else {
      ...
      console.log(
        chalk.greenBright(`Your current balance is: Rs.${myBalance}`)
      );
      ...
      if (selectAnswers.fastCash <= myBalance) {
        ...
        console.log(
          chalk.greenBright(
            `You have succfully withdraw an amount of: Rs.${selectAnswers.fastCash} Your new balance is: Rs.${myBalance}`
          )
        );
      } else {
      ...
      if (transferAmount.amount <= myBalance) {
        ...
        console.log(
          chalk.greenBright(
            `you have transfered an amount of: Rs.${chalk.cyanBright(
              transferAmount.amount
            )} to account No ${chalk.cyanBright(
              transferAmount.transfer
            )}.Your reamainig balance is: Rs.${chalk.cyanBright(myBalance)}`
          )
        );
      }
      ...
      let depositMoney = await inquirer.prompt([
        {
          name: "selectMoney",
          message: chalk.blue(
            "Please select the amount you want to deposit in your account."
          ),
          type: "list",
          choices: [500, 1000, 5000],
        },
      ...
      console.log(
        chalk.greenBright(
          `you have succefully deposited an amount of: Rs.${depositMoney.selectMoney} Your new balance is: Rs.${myBalance}`
        )
      );
